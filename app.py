...
mock_insight = {
    ...
    "timeline": [
        ...
        {"entity": "AI", "description": "뫼르소의 태도를 '사회적 거짓말을 거부하는 인간'으로 해석하는 관점 제시 (질문 방향: 철학적 접근)."},
...
with st.sidebar:
    ...
    save_period = st.number_input("세션 자동 저장 주기 (대화 턴 단위)", min_value=100, value=100, step=10)
    
    st.write("---")
    st.subheader("📜 히스토리 목록")
    # 가상의 과거 세션 목록
    past_sessions = ["Session_20260601_이방인", "Session_20260528_사피엔스", "Session_20260515_데미안"]
    selected_session = st.selectbox("과거 세션 로드", ["현재 세션 진행 중"] + past_sessions)
    if selected_session != "현재 세션 진행 중":
        st.info(f"💡 {selected_session} 데이터가 로드되었습니다. (프로토타입 기능)")
...
if st.session_state.step == 1:
    ...
    with c3:
        book_status = st.selectbox("읽기 상태", ["읽는 중", "다 읽음", "n회차 읽는 중"], index=["읽는 중", "다 읽음", "n회차 읽는 중"].index(st.session_state.book_info["status"]))
...
# -----------------------------------------------------------------------------
    ...
    # 관심사 태그 (st.multiselect 형태의 배지 표현)
    ...
    for idx, card in enumerate(mock_insight["directions"]):
        if card["keyword"] in selected_tags:  # 태그 필터링 반영
            with cols[idx]:
...
# -----------------------------------------------------------------------------
    ...
    st.title("🌐 Conbook — 3단계: 시각화 및 리마인더")
    ...
    with tab2:
        ...
        st.write("")
        # 수직 타임라인 카드 구현 (Markdown 구조 활용)
        for idx, tl in enumerate(mock_insight["timeline"]):
            # 프로토타입 필터링 조건 분기 모사
            if filter_keyword != "전체 보기" and idx % 2 == 1: 
                continue # 가상으로 홀수 인덱스만 걸러지게 연출
                
            icon = "👤" if tl["entity"] == "User" else "🤖"
            st.markdown(f"""
            <div style="background-color: #f0f2f6; padding: 15px; border-left: 5px solid #4f46e5; border-radius: 5px; margin-bottom: 12px;">
                <strong>{icon} {tl['entity']}_005</strong><br>
                <span style="color: #31333F;">{tl['description']}</span>
            </div>
            """, unsafe_allowed_html=True)
            
        st.write("---")
        
        # 원본 JSON 데이터 확인 공간 (기본 접힘)
    ...
    st.title("📜 Conbook — 4단계: 과거 기록 한눈에 보기")
